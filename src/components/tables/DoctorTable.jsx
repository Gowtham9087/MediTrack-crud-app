...
function DoctorTable({ doctors = [], startEdit, setDeleteId }) {
  ...
  const IdCellRenderer = (params) => {
    ...
    return (
      <span className="font-mono font-bold text-slate-400 dark:text-slate-500 flex items-center h-full text-sm">
        DR{String(rowNumber).padStart(3, "0")}
      </span>
    );
  };
  ...
  const columnDefs = useMemo(() => [
    { headerName: "Doctor ID", width: 120, cellRenderer: IdCellRenderer },
    { 
      headerName: "Doctor Name", 
      valueGetter: (params) => {
        const name = params.data?.name || "";
        if (!name.toLowerCase().startsWith("dr.") && !name.toLowerCase().startsWith("dr ")) return `Dr. ${name}`;
        return name;
      },
      flex: 1.5, minWidth: 200, cellClass: "flex items-center text-sm font-bold text-slate-900 dark:text-white" 
    },
    { headerName: "Specialization", field: "specialization", width: 160, cellClass: "flex items-center text-sm font-semibold text-slate-700 dark:text-slate-300 capitalize" },
    { headerName: "Email", field: "email", flex: 1.2, minWidth: 180, cellClass: "flex items-center text-sm text-slate-600 dark:text-slate-400" },
    { headerName: "Phone", field: "phone", width: 140, cellClass: "flex items-center font-mono text-sm text-slate-600 dark:text-slate-300" },
  ...
  return (
    <div className="ag-theme-quartz w-full custom-doctor-grid" style={{ "--ag-font-family": "Inter, system-ui, sans-serif", "--ag-font-size": "14.5px" }}>
      <style>{`
        .custom-doctor-grid {
